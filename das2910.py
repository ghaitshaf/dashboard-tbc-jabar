...
if selected == "Home":
    ...
    col2.metric("Rata-rata Kasus per Kabupaten", f"{mean_kasus:,.0f}")
    ...
    fig_hist = px.histogram(
        df, x="kasus_2024",
        nbins=10,
        title="Sebaran Jumlah Kasus antar Kabupaten",
        labels={"kasus_2024": "Kasus per Kabupaten"},
    ...
    **3️⃣ Gejala TBC di luar paru-paru (tergantung organ):**
    ...
    # ==============================
    st.subheader("1️⃣ Ukuran Frekuensi — Prevalensi")
    ...
    **Rumus:**  
    """)
    st.latex(r"\text{Prevalensi} = \frac{\text{Kasus TBC}}{\text{Populasi}} \times 100{,}000")

    # --- Hitung prevalensi provinsi secara keseluruhan
    total_kasus = df["kasus_2024"].sum()
    total_populasi = df["populasi_2024"].sum()
    prevalensi_provinsi = (total_kasus / total_populasi) * 100000

    st.metric("Prevalensi Provinsi Jawa Barat (per 100.000 penduduk)", f"{prevalensi_provinsi:,.2f}")

    st.dataframe(df[["kabupaten", "kasus_2024", "populasi_2024", "prevalensi_per_100k"]],
                 hide_index=True, use_container_width=True)

    st.markdown("---")

    # ==============================
    # Bagian 2 — Ukuran Asosiasi
    # ==============================
    st.markdown("---")
    st.subheader("2️⃣ Ukuran Asosiasi — PR, POR, dan AR")

    # --- Tabel 2x2: Kepadatan Penduduk vs TBC
    data_asosiasi = {
        "Kepadatan Penduduk": ["X > Rata-rata", "X < Rata-rata", "Total"],
        "TBC (+)": [82599, 142199, 224798],
        "TBC (−)": [22996, 50097396, 50120392],
        "Total": [105595, 50239595, 50345190]
    }
    ...
    a, b, c, d = 82599, 22996, 142199, 50097396

    # Prevalence Ratio (PR)
    CI_exposed = a / (a + b)
    CI_unexposed = c / (c + d)
    PR = CI_exposed / CI_unexposed
    ...
    with col1:
        st.metric(label="Prevalence Ratio (PR)", value=f"{PR:.3f}")
    with col2:
        st.metric(label="Prevalence Odds Ratio (POR)", value=f"{POR:.3f}")
    with col3:
        st.metric(label="Attributable Risk (AR)", value=f"{AR:.3f}")
    ...
    st.latex(r"AR = \frac{a}{a + b} - \frac{c}{c + d}")
    ...
    # Hitung persentase perubahan aman
    ...
    **Tahun:** 2025  
    ...
    Provinsi Jawa Barat, termasuk ukuran frekuensi penyakit (*prevalensi per 100.000 penduduk*)
    ...
    ### 🙏 Acknowledgement
    Penyusunan dashboard ini turut dibantu oleh **ChatGPT (OpenAI, model GPT-5)**  
    dalam proses penulisan kode, perancangan visualisasi, dan penyusunan teks deskriptif.  
    Seluruh hasil akhir telah diperiksa, disunting, dan disesuaikan oleh penulis.
