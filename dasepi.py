...
if selected == "Home":
    ...
    st.subheader("🗺️ Peta Persebaran Prevalensi TBC di Jawa Barat (2024)")

    # --- Load shapefile Jawa Barat
    shp_path = r"C:\Users\gshaf\OneDrive\Documents\Semester 5\Epidemiologi\newdasbor\shapefile\RBI_50K_2023_Jawa Barat.shp"
    gdf = gpd.read_file(shp_path)

    # --- Pastikan kolom nama kabupaten sama dengan di df
    # Misalnya: di shapefile kolomnya 'WADMKK' (ganti sesuai atribut shapefile kamu)
    gdf = gdf.rename(columns={"WADMKK": "kabupaten"})

    # --- Gabungkan dengan data prevalensi TBC
    gdf_merged = gdf.merge(df, on="kabupaten", how="left")
    ...
    folium.Choropleth(
        geo_data=gdf_merged,
        data=gdf_merged,
        columns=["kabupaten", "prevalensi_per_100k"],
        key_on="feature.properties.kabupaten",
        fill_color="YlOrRd",
        fill_opacity=0.8,
        line_opacity=0.5,
        legend_name="Prevalensi TBC (per 100.000 penduduk)"
    ).add_to(m)

    # --- Tambahkan label nama kabupaten
    for _, row in gdf_merged.iterrows():
        ...
        folium.map.Marker(
            [lat, lon],
            icon=folium.DivIcon(
                html=f"""<div style="font-size:8pt; color:#333;">{row['kabupaten']}</div>"""
            )
        ).add_to(m)
    ...
    **Rumus:**  
    ...
    # 2️⃣ UKURAN ASOSIASI
    # ==============================
    st.subheader("2️⃣ Ukuran Asosiasi — PR dan POR")
